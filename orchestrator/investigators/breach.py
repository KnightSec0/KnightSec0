...
class BreachInvestigator:
    ...
    async def run(self, emails: list[str], usernames: list[str]) -> dict:
        """Check for email/username in breach databases."""
        logger.info(f"Breach search for {len(emails)} emails, {len(usernames)} usernames")
        results = {}
            ...
            for email in emails:
                try:
                    async with session.get(
                        ...
                        auth=aiohttp.BasicAuth(settings.dehashed_api_login, settings.dehashed_api_key),
                        timeout=15
                    ) as resp:
                        if resp.status == 200:
            ...
            for query in queries[:10]:  # Limit to 10 queries
                try:
                    async with session.post(
                        ...
                        json={"term": query, "buckets": [], "lookuplevel": 0, "maxresults": 50, "timeout": 10},
